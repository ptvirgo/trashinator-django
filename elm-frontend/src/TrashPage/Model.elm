module TrashPage.Model exposing (GqlPageStats, GqlResponse(..), GqlSiteStats, GqlTrash, GqlUserStats, Jwt(..), Model, TPEntry, TPMeta, TPOptions, TPStats, TrashPage, WhichDay(..), emptyPage, jwtString, parsePageStats, parseSaveTrash, parseSiteStats, parseTrash, parseUserStats, relativeDate, setPageChanged, setPageDay, setPageError, setPageVolume, whichDayToString)
...
relativeDate ts whichDay =
    let
        ...
        year =
            toString (Date.year elmDate)

        month =
            monthString elmDate

        day =
            Date.day elmDate |> toString |> String.padLeft 2 '0'
    in
    Trash.Scalar.Date <| year ++ "-" ++ month ++ "-" ++ day


whichDayToString : WhichDay -> String
whichDayToString day =
    case day of
        Today ->
            "today"
...
type alias GqlPageStats =
    { user : GqlUserStats, site : GqlSiteStats }


type GqlResponse
    = TrashData (Maybe GqlTrash)
    | StatsData GqlPageStats
        ...
        field =
            case metric of
                ...
                Litres ->
                    TrashNode.litres
    in
    TrashNode.selection GqlTrash |> with field
...
parseUserStats metric =
    let
        field =
            case metric of
                Gallons ->
                    UserStatsNode.gallonsPerPersonPerWeek

                Litres ->
                    UserStatsNode.litresPerPersonPerWeek
    in
    UserStatsNode.selection GqlUserStats |> with field
...
parseSiteStats metric =
    ...
    in
    SiteStatsNode.selection GqlSiteStats
        |> with volField
        |> with devField


parsePageStats : Metric -> SelectionSet GqlPageStats Trash.Object.StatsNode
parsePageStats metric =
    StatsNode.selection GqlPageStats
...
parseSaveTrash metric =
    SaveTrash.selection identity
